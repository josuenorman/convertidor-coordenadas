...
(function(){
  function moveAndClean(){
    ...
    if(manual){manual.classList.add('manual-top-horizontal');manual.style.order='1';}
    if(mapSection){mapSection.classList.add('map-after-manual');mapSection.style.order='2';}
    ...
    if(redundantSearch){
      redundantSearch.value='';
      redundantSearch.setAttribute('aria-hidden','true');
  ...
  function addStyles(){
    ...
    s.textContent=`
      ...
      .main{display:flex!important;flex-direction:column!important;gap:10px!important}.main>section{width:100%!important;grid-column:1/-1!important}.manual-top-horizontal{order:1!important}.map-after-manual{order:2!important}.table-under-map{order:3!important}#gisModule{order:4!important}
      ...
      .map-after-manual .map{height:60vh!important;min-height:500px!important}.hidden-map-filter-search,#search.hidden-map-filter-search{display:none!important}
