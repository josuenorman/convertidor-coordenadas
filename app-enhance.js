...
(function(){
  ...
  function addPointWithoutRecentering(point){
    const p=pr();
  ...
  window.toggleCaptureMode=function(){captureMode=!captureMode;const b=document.getElementById('captureBtn');if(b){b.textContent=captureMode?'✅ Captura activa':'📌 Capturar punto';b.classList.toggle('green',captureMode);b.classList.toggle('out',!captureMode);}const map=document.getElementById('map');if(map){map.style.cursor=captureMode?'crosshair':'grab';}msg(captureMode?'Modo captura activo: cada clic en el mapa guardará un punto sin mover la vista.':'Modo navegación activo: puedes mover, buscar y hacer zoom sin capturar puntos.')};
  ...
  window.useLocation=function(){
    ...
    navigator.geolocation.getCurrentPosition(pos=>{
      ...
      try{addPointWithoutRecentering({name:'Mi ubicación',lat,lon,...ll2utm(lat,lon),source:'Ubicación',notes:'Capturada con geolocalización del navegador'});}catch{drawMap();}
      view.lat=lat;view.lon=lon;view.z=17;drawMap();
      msg('Mapa centrado en tu ubicación. Precisión aproximada: '+Math.round(pos.coords.accuracy||0)+' m.');
  ...
  window.selectPlace=function(i){
    ...
    const input=document.getElementById('placeSearch'); if(input) input.value=(r.display_name||'').split(',').slice(0,2).join(', ');
    msg('Mapa centrado en: '+(r.display_name||'lugar seleccionado'));
  };

  window.searchPlace=async function(silent=false){
    ...
    if(!silent)msg('Buscando coincidencias para: '+q+' ...');
    try{
      ...
      if(!silent)msg(data.length?('Encontré '+data.length+' resultado(s). Elige uno de la lista.'):('No encontré ese lugar.'));
    }catch(e){if(!silent)msg('No se pudo buscar el lugar. Revisa conexión o intenta de nuevo.');}
  };
  ...
  function captureAtEvent(e){
    const r=document.getElementById('map').getBoundingClientRect(),w=document.getElementById('map').clientWidth||900,s=256,tlx=tx(view.lon,view.z)*s-w/2,tly=ty(view.lat,view.z)*s-470/2;
    const wx=(tlx+e.clientX-r.left)/s,wy=(tly+e.clientY-r.top)/s,lat=latFrom(wy,view.z),lon=lonFrom(wx,view.z);
    addPointWithoutRecentering({name:'Mapa',lat,lon,...ll2utm(lat,lon),source:'Mapa',notes:'Capturada en mapa'});
    msg('Coordenada capturada: '+lat.toFixed(6)+', '+lon.toFixed(6)+'. Captura sigue activa.');
  }

  window.mapDown=function(e){dragLocal={x:e.clientX,y:e.clientY,startLat:view.lat,startLon:view.lon,moved:false};document.getElementById('map')?.classList.add('grabbing')};
  window.mapMove=function(e){if(!dragLocal)return;const dx=e.clientX-dragLocal.x,dy=e.clientY-dragLocal.y;if(Math.abs(dx)+Math.abs(dy)>4)dragLocal.moved=true;const s=256,cx=tx(dragLocal.startLon,view.z)*s-dx,cy=ty(dragLocal.startLat,view.z)*s-dy;view.lon=lonFrom(cx/s,view.z);view.lat=latFrom(cy/s,view.z);drawMap()};
  window.mapUp=function(e){document.getElementById('map')?.classList.remove('grabbing');if(!dragLocal)return;const moved=dragLocal.moved;dragLocal=null;if(moved)return;if(captureMode)captureAtEvent(e);else msg('Modo navegación activo. Activa “Capturar punto” para guardar coordenadas desde el mapa.')};
  window.mapClick=function(e){if(captureMode)captureAtEvent(e)};
